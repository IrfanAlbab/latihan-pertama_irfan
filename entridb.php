<?php
	...
	// koneksi ke database
	$mysqli = new mysqli("localhost", "root", "01695", "daftar angkot");
	
	$arrFile = array("AJG.txt", "AT.txt", "GL.txt", "JDM.txt", "JPK.txt", "LDG.txt", "LG.txt", "MKS.txt");
	
	function bacaFile($namafile) {
		
		global $dir, $mysqli;
		
		$file = fopen(getcwd()."/tmp/".$namafile, "r");
		$baris = fgets($file);
		...
		while(($baris = fgets($file)) ==! FALSE) {
			...
			if (count($data) > 3) {
				...
				$strSQL = "INSERT INTO rute SET angkot_id = '$angkot_id', rute_name = '$strJalan', lattitude = '$latitude', longitude = '$longitude'";
				$rst = $mysqli->query($strSQL);
